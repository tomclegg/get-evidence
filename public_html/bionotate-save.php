...
$variant_name = evidence_get_variant_name($_GET['variant_id'], '-', true);
$bionotate_key = $_GET['article_pmid'] . '-' . $variant_name;
if (isset($_POST['xml']))
  $xml = $_POST['xml'];
else {
...
$snap_row = theDb()->getRow ("SELECT * FROM snap_latest WHERE variant_id=? AND article_pmid=? AND genome_id=0 AND disease_id=0 LIMIT 1",
                         array ($_GET['variant_id'], $_GET['article_pmid']));
...
$edit = theDb()->getRow ("SELECT * FROM edits WHERE variant_id=? AND article_pmid=? AND genome_id=0 AND disease_id=0 AND edit_oid=? AND previous_edit_id=? ORDER BY edit_id DESC LIMIT 1",
			 array ($_GET['variant_id'], $_GET['article_pmid'], getCurrentUser('oid'), $snap_row['edit_id']));
...
header("Location: /{$variant_name}#a".$_GET['article_pmid']);
