...
for ($i=1; $i<$_SERVER["argc"]; $i++) {
    ...
    reconnectDb();
    $q = theDb()->query ("DROP TABLE import_1000genomes_onefile");
    $q = theDb()->query ("CREATE TABLE import_1000genomes_onefile LIKE import_1000genomes");
    ...
    print "Merging...";
    reconnectDb();
    $q = theDb()->query ("INSERT INTO import_1000genomes SELECT * FROM import_1000genomes_onefile ON DUPLICATE KEY UPDATE import_1000genomes.occur=import_1000genomes.occur+VALUES(occur)");
    if (theDb()->isError($q)) print $q->getMessage();
    print theDb()->affectedRows();
    print "\n";
